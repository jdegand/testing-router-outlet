...
describe('AppComponent', () => {
  ...
  describe('Given no search criteria', () => {
    ...
    */
    ...
    it('Then shows error message and disabled button', async () => {
      ...
      expect(screen.queryByText('Search criteria is required!')).toBeInTheDocument();
  ...
  describe('Given a search criteria with no book match', () => {
    it('Then shows No book found', async () => {
      ...
      expect(screen.queryByText('Search criteria is required!')).not.toBeInTheDocument();

      await userEvent.click(button);

      expect(screen.queryByText('No book found for this search')).toBeInTheDocument();
  ...
  describe('Given a search criteria with one book match', () => {
    it('Then shows One book and no error', async () => {
      ...
      expect(screen.queryByText('Borrowed Book: 1984 by George Orwell')).toBeInTheDocument();
    });
  });
  ...
  describe('Given a search criteria in Uppercase with one book match', () => {
    it('Then shows One book and no error', async () => {
      ...
      expect(screen.queryByText('Borrowed Book: Animal Farm by George Orwell')).toBeInTheDocument();
    });
  });
  ...
  describe('Given a search criteria with multple books matches', () => {
    it('Then shows a list of books', async () => {
      ...
      expect(screen.queryByText('Borrowed Book: The Lord of the Rings by J.R.R. Tolkien')).toBeInTheDocument();
      expect(screen.queryByText('Borrowed Book: The Hobbit by J.R.R. Tolkien')).toBeInTheDocument();
